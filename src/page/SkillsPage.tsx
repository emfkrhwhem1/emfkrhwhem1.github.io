...
function SkillsPage() {
  ...
  return (
    <StepPage title="Skills" stepNumber={3}>
      <div className="text-center mb-8 text-gray-400">
        프론트엔드 개발에 필요한 다양한 기술들을 보유하고 있습니다.
      </div>
      ...
      <div className="grid grid-cols-3 md:grid-cols-5 gap-4 md:gap-6 w-full max-w-5xl mx-auto px-4">
        {filteredSkills.map(skill => {
          const IconComponent = skill.Icon;
          return (
            <div
              key={skill.name}
              className="group flex flex-col items-center justify-center p-4 md:p-6 bg-white/5 border border-white/10 rounded-2xl cursor-pointer transition-all duration-300 ease-out 
                hover:-translate-y-1 md:hover:-translate-y-2 hover:bg-white/10 hover:border-white/30 hover:shadow-xl"
            >
              <div
                className="w-12 h-12 md:w-16 md:h-16 flex items-center justify-center bg-black/40 rounded-xl md:rounded-2xl mb-3 md:mb-4 transition-all duration-300 ease-out 
                group-hover:scale-110 group-hover:bg-black/60 shadow-inner"
              >
                {/* 
                  핵심: colorClass 내부의 문자열을 그대로 사용합니다. 
                  동적으로 생성하지 않고 정의된 문자열을 쓰기 때문에 Tailwind가 인식합니다.
                */}
                <IconComponent
                  className={`text-3xl md:text-4xl transition-colors duration-300 ${skill.colorClass}`}
                />
              </div>
              <span className="text-[10px] md:text-sm font-medium text-gray-300 md:text-gray-400 transition-colors duration-300 group-hover:text-white text-center break-keep">
                {skill.name}
              </span>
            </div>
          );
